...
const FamilyLifeAndHealthInsurance = ({familyInsuranceNotAdviserList, setFamilyInsuranceNotAdviserList, clientID, totalFamilyInsuranceNotAdviserCurrentAccountValue }) => {
    ...
    const openEditModal = (flahi_id) => {
        ...
        setEditData(familyInsuranceNotAdviserList.find((item)=>item.flahi_id === flahi_id))
    };
    ...
    const submitEditData = async (e) => {
        ...
        const res = await axios.post(`${API_BASE_URL}/newLifeHealthInsuranceNotAdviser`, data);

        if (res.status === 200) {
            setFamilyInsuranceNotAdviserList(res.data.Life_And_Health_Insurance)
        }
    }
        ...
        <div className="mt-4 ml-2">
            <div className='flex items-center gap-4'>
                <button className='px-4 py-2 bg-gray-200 flex items-center gap-4 font-bold text-slate-600'>
                    FAMILY LIFE AND HEALTH INSURANCE <IoIosInformationCircle className='text-lg' />
                </button>
                <FaSyncAlt />
            </div>
            ...
            <div className="overflow-x-auto mt-4">
                <table className="w-auto table-auto divide-y divide-gray-200 text-sm text-left text-gray-500">
                    ...
                    <tbody className='divide-y divide-gray-200 '>
                        {familyInsuranceNotAdviserList.map((item, idx) => (
                    ...
                    <tfoot className='divide-y divide-gray-200'>
                        <tr className=" text-gray-900">
                            ...
                            <td className="px-6 py-3 text-xs font-bold">{totalFamilyInsuranceNotAdviserCurrentAccountValue?.toLocaleString('en-US')}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
export default FamilyLifeAndHealthInsurance