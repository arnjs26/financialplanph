...
const StocksInNonListedCompany = ({ stocksNonListedList, familyCompositionList, clientID, handleInputChange, handleAddInputForm, removeOtherInputFields, totalStocksNonListedEstimatedValue }) => {
        ...
        <div className="mt-4 ml-2">
            <div className='flex items-center gap-4'>
                <button className='px-4 py-2 bg-gray-200 flex items-center gap-4 font-bold text-slate-600'>
                    Stocks In Non-Listed Companies <IoIosInformationCircle className='text-lg' />
                </button>
                <FaSyncAlt />
            </div>
                ...
                <table className="w-full table-auto divide-y divide-gray-200 text-sm text-left text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-200">
                        <tr className='text-center'>
                            ...
                            <th scope='col' className="p-2 font-bold text-slate-600"><FaPlusCircle onClick={() => handleAddInputForm('stocksNonListed')} className='text-lg cursor-pointer' /></th>
                            ...
                            <tr key={idx} className="hover:bg-gray-100 text-xs text-gray-700">
                                <th scope="row" className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap ">
                                    <input
                                        ...
                                        onChange={(e) => handleInputChange(e, idx, 'stocksNonListed', null)}
                                        ...
                                        item.heirs.map((heir, heirIndex) => (
                                            <span key={heirIndex} className='flex items-center gap-2'>
                                                <select
                                                    ...
                                                    onChange={(e) => handleInputChange(e, idx, 'heirsStocksNonListed', heirIndex)}
                                                    className="p-2 w-[150px] rounded-md border border-gray-500 text-gray-700 text-xs"
                                                >
                                                    <option value="">Please select</option>
                                ...
                                {familyCompositionList.length !== 0 &&
                                    <td className="px-4 py-2">
                                        <button
                                            onClick={() => handleAddInputForm('heirsStocksNonListed', idx)}
                    ...
                    <tfoot className='divide-y divide-gray-200'>
                        <tr className=" text-gray-900">
                            <th scope="row" className="text-xs px-6 py-3">Total Stocks in Non-Listed Companies</th>
                            <td className="px-6 py-3 text-xs font-bold">{totalStocksNonListedEstimatedValue?.toLocaleString('en-US')}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
export default StocksInNonListedCompany